...
const Dashboard = () => {
  ...
  const atamaKaldir = (depocuId, kargoid) => {
    ...
    dispatch(
      kargoAtamasiKaldir({
        user_id: depocuId,
        kargo_id: kargoid,
        table_name: 'tbl_tirci_atamalari',
      }),
    )
  }
  ...
  React.useEffect(() => {
    ...
    console.log('atanacak kargolar', atacanakKargolar)
  ...
  console.log(selectMessageState)
  const columns = [
    {
      ...
      value: (item) => (
        <div>
          <div className="h-8 text-xl">
            ...
            {
              atacanakKargolar.filter((item) => {
                if (item.atanan_kullanici == null) {
                  return item
                }
                if (item.atanan_kullanici != null && item.atanan_kullanici == params.id) {
                  return item
                }
              })?.length
            }
  ...
  return (
    <>
      <AtamaModal open={openAtamaModal} setOpen={setOpenAtamaModal} title="Nakliyeci Atama">
        <CListGroup>
      ...
      <div>
        ...
        <DataTable
          ...
          columns={columns}
          data={atacanakKargolar.filter((item) => {
            if (item.atanan_kullanici == null) {
              return item
            }
            if (item.atanan_kullanici != null && item.atanan_kullanici == params.id) {
